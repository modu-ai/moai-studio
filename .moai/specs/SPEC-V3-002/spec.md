...
labels: [phase-2, terminal, pty, libghostty, rust]
revision: v1.2.0 (plan-auditor 2026-04-24 감사 교정 — terminal-spike → ghostty-spike, clipboard arboard, frontmatter schema)
---
...
| 1.2.0 | 2026-04-24 | plan-auditor 감사 결과 반영: (C-001) example 이름 `terminal-spike` → `ghostty-spike` 전수 교정 (10회). 실제 `crates/moai-studio-terminal/Cargo.toml` + `examples/ghostty-spike.rs` 와 일치. (m-002) clipboard crate `copypasta 0.10` → `arboard 3.0` 으로 §6.1 교체 (실제 Cargo.toml 반영). (m-001) frontmatter `created/updated → created_at/updated_at` + `labels` / `revision` 추가. §7 test 파일명 drift (M-001/M-002) 는 실 테스트 목록 전수 갱신으로 별도 sync phase 예정. |
...
성공 기준: macOS 14 + Ubuntu 22.04 runner 상에서 `cargo run -p moai-studio-terminal --example ghostty-spike` 가 GPUI 윈도우에 `$SHELL` prompt 를 렌더하고, 사용자 키 입력이 shell 까지 전달되어 명령 결과가 다시 Grid 에 표시되는 end-to-end 동작.
...
[Event-Driven] PR 이 생성되면 CI 는 SPEC-V3-001 의 기존 4 gate 에 더해 `cargo build -p moai-studio-terminal --example ghostty-spike` 를 추가 gate 로 **실행해야 한다**.
...
| AC-T-4 | RG-2 | PTY spawn 가능 환경 | `cargo run -p moai-studio-terminal --example ghostty-spike` | GPUI 윈도우가 열리고 `$SHELL` prompt 렌더됨 (예: `zsh %` 또는 `bash $`) |
| AC-T-5 | RG-2, RG-4 | ghostty-spike 윈도우 실행 중 | 사용자가 `echo hello<Enter>` 입력 | 3 초 이내 "hello" 라인이 Grid 에 표시됨 |
| AC-T-6 | RG-2 | ghostty-spike 윈도우 실행 중 | 사용자가 shell 에 `exit<Enter>` | 1 초 이내 윈도우 종료, open FD 수가 spawn 전 기준선 이하로 복귀 (lsof 측정) |
...
| AC-T-10 | RG-4 | ghostty-spike 실행 중, shell prompt 표시 | Cmd+C (macOS) 누름, selection 비어있음 | SIGINT 가 PTY 에 전송되어 shell 에 `^C` 표시, 시스템 클립보드 값은 변하지 않음 |
| AC-T-11 | RG-4 | ghostty-spike 실행 중, 마우스로 텍스트 선택 | Cmd+C (macOS) / Ctrl+Shift+C (Linux) 누름 | 선택된 텍스트가 OS 클립보드에 복사됨 (`pbpaste` / `xclip -o` 로 검증) |
| AC-T-12 | RG-5 | PR 이 이 SPEC 의 구현 커밋 포함 | GitHub Actions CI 트리거 | macOS + Linux 양쪽에서 Zig 설치 스텝 + 기존 4 gate + `ghostty-spike` 빌드 gate 모두 통과 |
...
| `arboard` | `3.0` (crates.io) | 클립보드. v1.2.0 `copypasta 0.10` → `arboard 3.0` 으로 교체 (Run Phase 실증). 다중 플랫폼 지원 + Ctrl+Shift+C (Linux) / Cmd+C (macOS) 선택 사본. |
...
- `cargo build -p moai-studio-terminal --example ghostty-spike` (링크 성공만 검증, headless 불가능한 GPUI 부분은 skip)
...
- macOS: `cargo run -p moai-studio-terminal --example ghostty-spike` → zsh prompt → `ls`, `echo`, `exit` 시나리오
