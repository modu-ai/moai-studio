...
revision: v1.0.0 (initial draft, USER-DECISION-PK-B (b) 미보유 결정의 후속 무료 채널 등록 SPEC)
---
...
**상태**: PENDING. MS-1 진입 시 결정 필요.

옵션:

- (a) **권장 (단기): `modu-ai/homebrew-tap` custom tap 신설** — 사용자는 `brew tap modu-ai/tap && brew install --cask moai-studio` 2 명령. 즉시 publish 가능. 통제 100%. 단점: tap 추가 1 단계 마찰.
- (b) `homebrew/homebrew-cask` 본가 PR — 사용자는 `brew install --cask moai-studio` 1 명령. 마찰 0. 단점: 본가 review 시간 (수일~수주) 필요. cask 의 audit 기준 (versioned URL + license + 업스트림 active 등) 만족 필요.

**기록 위치**: 본 SPEC 의 plan.md 또는 별 progress.md 에 결정 후 기록.
...
- (a) **권장 (단기): `modu-ai/scoop-bucket` custom bucket 신설** — 사용자는 `scoop bucket add moai https://github.com/modu-ai/scoop-bucket && scoop install moai-studio` 2 명령. 통제 100%.
- (b) `ScoopInstaller/Extras` 본가 PR — 사용자는 `scoop install moai-studio` 1 명령. 본가 review 필요.
...
- (a) **권장: 자동화 (release.yml step 추가)** — 새 tag publish 시 GitHub Actions 가 cask / scoop / AUR 모두 자동 PR 생성 또는 commit. maintainer 부담 0. 초기 setup 1회 필요. `homebrew-bump-cask-pr@v3` 등 외부 action 사용.
- (b) 수동 — 매 release 마다 maintainer 가 cask / scoop / AUR PKGBUILD 의 `version` / `sha256` 을 직접 수정. setup 부담 0. release 마다 작업 부담.
...
Status: draft (USER-DECISION-DIST-A/B/C 결정 후 ready 단계 진입)
